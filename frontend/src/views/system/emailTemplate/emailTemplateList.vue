...
import infoModel from './modules/emailTemplateModel'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return this.$http.get('/sysEmailTemplate/page', { params: requestParameters }).then(res => {
          return res.result
        })
      },
      selectedRowKeys: [],
  ...
  methods: {
    remove (key) {
      this.$http.delete('/sysEmailTemplate', { data: { Ids: !key.length ? [key] : key } }).then(res => {
        if (res.code === 200) {
