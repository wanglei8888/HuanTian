...
import infoModel from './modules/appsModel'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return this.$http.get('/sysApps/page', { params: requestParameters }).then(res => {
          return res.result
        })
      },
      selectedRowKeys: [],
  ...
  methods: {
    remove (key) {
      this.$http.delete('/sysApps', { data: { Ids: !key.length ? [key] : key } }).then(res => {
        if (res.code === 200) {
